...
/**
 * Clean Firebase errors into user-friendly messages
 */
export const cleanFirebaseError = (errorString) => {
  if (!errorString) return '';
  if (typeof errorString !== 'string') return errorString.message || 'An error occurred';
  ...
  return errorString.replace(/^Firebase:\s*/, '').replace(/Error\s*\((.*?)\)/g, '$1');
};