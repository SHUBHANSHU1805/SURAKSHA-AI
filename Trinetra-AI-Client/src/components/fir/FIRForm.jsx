...
const FIRForm = ({ onClose }) => {
  const { t } = useTranslation();
  const { submitFIR } = useFIR();
  ...
  // Fetch suggestions from Nominatim (debounced via useEffect)
  const fetchSuggestions = async (query) => {
    ...
    try {
      const searchQuery = query.toLowerCase().includes("india")
        ? query
        : `${query}, India`;
  ...
  const canSubmit = formData.location && formData.type && formData.severity && coordinates;
      ...
      <div className={`rounded-lg shadow-md p-8 ${bgCard}`}>
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <div className="relative">
              ...
              {showSuggestions && addressSuggestions.length > 0 && (
                <div
                  className={`absolute top-full left-0 right-0 mt-1 border rounded-lg shadow-lg z-50 max-h-48 overflow-y-auto ${bgCard} ${borderColor}`}
                >
                  {addressSuggestions.map((suggestion, index) => (
                    <button
                      ...
                      className={`w-full text-left px-4 py-3 transition border-b last:border-b-0 ${
          ...
          <button
            ...
            className={`w-full font-semibold py-3 rounded-lg transition shadow-md flex items-center justify-center gap-2 ${
          ...
          >
            ...
            {submitting ? "Submitting..." : t("fir.submit") || "Submit FIR"}
          </button>

          {!canSubmit && (
            <p className={`text-xs text-center ${textTertiary}`}>
              Please fill Crime Type, Severity, and detect/enter a location to submit FIR
            </p>
          )}
        </div>
      </div>
