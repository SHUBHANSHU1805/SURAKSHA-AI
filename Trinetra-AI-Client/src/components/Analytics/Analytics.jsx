...
const Analytics = ({ darkMode, t }) => {
  ...
  useEffect(() => {
    const fetchOverall = async () => {
      try {
        const res = await fetch(`${apiUrl}/analytics/overall`);
  ...
  useEffect(() => {
    ...
    if (stateDetails[selectedState]) return;

    const fetchStateDetails = async () => {
      setLoadingState(true);
      try {
        const res = await fetch(`${apiUrl}/analytics/states/${encodeURIComponent(selectedState)}/details`);
        const json = await res.json();
        if (json.success) {
          setStateDetails(prev => ({
            ...prev,
            [selectedState]: json.data
          }));
        }
      } catch (err) {
        console.error(`Error fetching stats for ${selectedState}:`, err);
      } finally {
  ...
  const normalizeStateName = (name) => {
    ...
    if (norm === 'andaman and nicobar islands' || norm === 'andaman & nicobar') return 'andaman & nicobar';
    if (norm === 'jammu and kashmir' || norm === 'jammu & kashmir') return 'jammu & kashmir';
    if (norm === 'dadra and nagar haveli and daman and diu' || norm === 'dadra & nagar haveli and daman & diu') return 'dadra & nagar haveli and daman & diu';
    if (norm === 'delhi' || norm === 'the government of nct of delhi') return 'delhi';
    return norm;
  };
  ...
  const getWomenSafetyZones = (state) => {
    if (stateDetails[state] && stateDetails[state].women_safety_zones) {
      return stateDetails[state].women_safety_zones;
    }
    const safetyData = {
      // States
      'Andhra Pradesh': [
  ...
  const getPatrolRecommendations = (state) => {
    if (stateDetails[state] && stateDetails[state].patrol_recommendations) {
      return stateDetails[state].patrol_recommendations;
    }
    const patrolData = {
      // States
      'Andhra Pradesh': [
  ...
  const colors = ['#ef4444', '#f97316', '#eab308', '#3b82f6', '#8b5cf6', '#10b981', '#ec4899'];
  const histogramData = overallStats && overallStats.by_type ? Object.entries(overallStats.by_type)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 5)
    .map(([type, count], idx) => ({
      type,
  ...
  const maxCount = Math.max(...histogramData.map(d => d.count)) || 4000;
  const yLabels = [
    Math.round(maxCount),
  ...
  let currentOffset = 0;
  const pieData = overallStats && overallStats.by_severity ? severities.map(label => {
    const count = overallStats.by_severity[label] || 0;
    const total = Object.values(overallStats.by_severity).reduce((sum, v) => sum + v, 0) || 1;
    const percentage = count / total;
    const segmentLength = percentage * 502.65;
    const item = {
      label,
      count,
      color: severityColors[label],
      dasharray: `${segmentLength.toFixed(2)} 502.65`,
  ...
  const statesData = staticStatesData.map(state => {
    const normName = normalizeStateName(state.name);
    let count = 0;
    if (overallStats && overallStats.by_state) {
      const matchKey = Object.keys(overallStats.by_state).find(k => normalizeStateName(k) === normName);
      if (matchKey) {
        count = overallStats.by_state[matchKey];
      }
    }
    return {
      ...state,
      totalCrimes: count || state.totalCrimes
    };
  });

  const getCitiesForState = (state) => {
    if (stateDetails[state] && stateDetails[state].cities) {
      return stateDetails[state].cities;
    }
    const cityData = {
      'Andhra Pradesh': [
  ...
  return (
    <div className="space-y-6">
            ...
            <div className="w-full flex justify-center h-72 relative">
              <svg viewBox="0 0 200 200" className="w-full h-full max-w-sm" style={{ filter: 'drop-shadow(0 4px 6px rgba(0,0,0,0.1))' }}>
                ...
                <g pointerEvents="none">
                  ...
                  >
                    {overallStats ? overallStats.total_crimes.toLocaleString() : '10,975'}
                  </text>
                  <text
                    x="100"
                    y="108"
      ...
      <div className={`rounded-lg shadow-md p-6 ${bgCard}`}>
        ...
        {selectedState && (
          <div className="mt-6 space-y-4 animate-fadeIn relative min-h-[200px]">
            {loadingState && (
              <div className="absolute inset-0 bg-gray-900/10 dark:bg-black/20 backdrop-blur-[2px] flex items-center justify-center rounded-xl z-10" style={{ pointerEvents: 'all' }}>
                <div className="flex flex-col items-center gap-2 p-4 bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700">
                  <div className="w-8 h-8 border-4 border-blue-500 border-t-transparent rounded-full animate-spin"></div>
                  <span className="text-sm font-semibold text-gray-700 dark:text-gray-200">Analyzing crime database...</span>
                </div>
              </div>
            )}
